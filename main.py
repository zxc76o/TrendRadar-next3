...
except ImportError:  # pragma: no cover - 可选依赖在运行时检查
    BeautifulSoup = None
...
def load_config():
    ...
    x_trends_config = config_data.get("x_trends", {}) or {}
    default_location = x_trends_config.get("location", "worldwide") or "worldwide"
    default_location = str(default_location).strip().strip("/")
    default_max_topics = (
        int(x_trends_config.get("max_topics", 20)) if x_trends_config else 20
    )

    config["X_TRENDS_LOCATION"] = (
        os.environ.get("X_TRENDS_LOCATION", "").strip().strip("/")
        or default_location
        or "worldwide"
    )
    config["X_TRENDS_MAX_TOPICS"] = max(
        1,
        int(
            os.environ.get("X_TRENDS_MAX_TOPICS", "").strip()
            or default_max_topics
            or 20
        ),
    )
...
class DataFetcher:
    ...
    def fetch_data(
        ...
        if id_value == "x-hot":
            return self.fetch_x_trends(alias)
    ...
    def fetch_x_trends(self, alias: str) -> Tuple[Optional[str], str, str]:
        """抓取 X 平台热点并转换为统一结构"""
        location = CONFIG.get("X_TRENDS_LOCATION", "worldwide")
        location = str(location or "").strip().strip("/")
        if location:
        ...
        try:
            ...
            items = self._extract_x_trends_items(response.text)

            if not items:
                raise ValueError("未解析到任何热点")
            ...
            return json.dumps(payload, ensure_ascii=False), "x-hot", alias
        except Exception as exc:
            print(f"获取 X 热点失败: {exc}")
            return None, "x-hot", alias

    def _extract_x_trends_items(self, html: str) -> List[Dict]:
        """解析 trends24 页面内容"""
        limit = CONFIG.get("X_TRENDS_MAX_TOPICS", 20)
        if BeautifulSoup is not None:
            try:
                ...
                if card:
                    list_container = card.select_one(".trend-card__list")
                if not list_container:
                    list_container = soup.select_one("ol.trend-card__list")

                if list_container:
                    return self._extract_from_dom(list_container, limit)
            except Exception as exc:
                print(f"BeautifulSoup 解析 X 热点失败: {exc}")

        return self._extract_with_regex(html, limit)

    def _extract_from_dom(self, list_container, limit: int) -> List[Dict]:
        """使用 DOM 解析趋势数据"""
        items = []
        seen_titles = set()
        for li in list_container.select("li"):
            ...
            link = self._normalize_x_link(anchor.get("href", "").strip())

            tweet_count_el = li.select_one(".tweet-count")
            tweet_count = tweet_count_el.get_text(strip=True) if tweet_count_el else ""
    ...
    def _extract_with_regex(self, html: str, limit: int) -> List[Dict]:
        """在缺少 BeautifulSoup 时通过正则解析"""
        list_match = re.search(
            r"<ol class=trend-card__list>(.*?)</ol>", html, re.S | re.IGNORECASE
        )
        if not list_match:
            return []

        list_html = list_match.group(1)
        item_pattern = re.compile(r"<li>(.*?)</li>", re.S | re.IGNORECASE)
        items = []
        seen_titles = set()

        for match in item_pattern.finditer(list_html):
            li_html = match.group(1)
            anchor_match = re.search(
                r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', li_html, re.S | re.IGNORECASE
            )
            if not anchor_match:
                continue

            link = self._normalize_x_link(unescape(anchor_match.group(1).strip()))
            title = unescape(self._strip_html(anchor_match.group(2))).strip()
            if not title or title in seen_titles:
                continue
            seen_titles.add(title)

            tweet_count_match = re.search(
                r'class=tweet-count[^>]*data-count=([0-9]+)', li_html
            )
            tweet_count = ""
            if tweet_count_match:
                count_value = tweet_count_match.group(1)
                tweet_count = count_value
    ...
    @staticmethod
    def _normalize_x_link(link: str) -> str:
        """构造完整的热点链接"""
        if not link:
            return ""
        link = link.strip()
        ...
        if link.startswith("http://") or link.startswith("https://"):
            return link
        return urljoin("https://trends24.in", link)
